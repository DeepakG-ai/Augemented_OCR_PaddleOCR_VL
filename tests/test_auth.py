...
class AuthHttpTests(unittest.TestCase):
    ...
    def test_login_inactive_user_returns_401(self) -> None:
        record = {"id": "u-1", "email": "a@x.com", "hashed_pw": hash_password("p"),
        ...
        with patch.object(main.db_mod, "get_user_by_email", new=AsyncMock(return_value=record)):
            r = self.client.post("/auth/login", json={"email": "a@x.com", "password": "p"})
        self.assertEqual(r.status_code, 401)

    def test_login_wrong_password_returns_401(self) -> None:
        record = {"id": "u-1", "email": "a@x.com", "hashed_pw": hash_password("right"),
                  "role": "client", "is_active": True, "created_at": None}
        with patch.object(main.db_mod, "get_user_by_email", new=AsyncMock(return_value=record)):
            r = self.client.post("/auth/login", json={"email": "a@x.com", "password": "wrong"})
        self.assertEqual(r.status_code, 401)
    ...
    def test_me_with_valid_override_returns_user(self) -> None:
        record = {"id": "u-1", "email": "a@x.com", "role": "client",
    ...
    def test_admin_post_vendor_with_user_id_succeeds(self) -> None:
        user_uuid = "00000000-0000-0000-0000-00000000000a"
        row = {"id": "NEWCO", "name": "Newco Ltd", "status": "idle",
               "user_id": user_uuid, "created_at": "2026-01-01T00:00:00Z"}
        with patch.object(main.db_mod, "create_vendor_by_name", new=AsyncMock(return_value=row)), \
             ...
             patch.object(main.db_mod, "get_user_by_id", new=AsyncMock(return_value={"id": user_uuid})):
            r = self.client.post(
                "/vendors",
                json={"name": "Newco Ltd", "user_id": user_uuid},
            )
        self.assertEqual(r.status_code, 200)
